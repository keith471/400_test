...
LocalRegistry.prototype.register = function() {
    ...
    if (this.machType === constants.globals.NodeType.DEVICE) {
        ...
        this._respondToQueries(this);
        setInterval(this._respondToQueries, constants.localStorage.queryResponseInterval, this);
    } else {
        this._addCloud(data);
...
LocalRegistry.prototype.query = function(senderId, receiverId, cb) {
    ...
    // get the response
    this._getResponse(senderId, receiverId, cb, constants.localStorage.queryRetries, this);
}

/**
 * Try to get the response to a query up to retries times
 */
LocalRegistry.prototype._getResponse = function(senderId, receiverId, cb, retries, self) {
    var receiverQueries = JSON.parse(self.localStorage.getItem(receiverId));
    var response = receiverQueries[senderId];
    if (response !== null) {
        delete receiverQueries[senderId];
        self.localStorage.setItem(receiverId, JSON.stringify(receiverQueries));
    ...
    setTimeout(self._getResponse, 100, senderId, receiverId, cb, retries, self);
}
 ...
 * TODO I don't think this is thread-safe
 */
LocalRegistry.prototype._respondToQueries = function(self) {
    var queries = JSON.parse(self.localStorage.getItem(self.id));
    var connectionData = {
        port: self.port,
        ip: self.ip
    };
    ...
    self.localStorage.setItem(self.id, JSON.stringify(queries));
}
